...
things out. It is expensive to keep sockets open and it is even more expensive to create new sockets. uBlock Origin waits 2 seconds 
between two requests and that just does not make sense, therefore, `manualUpdateAssetFetchPeriod` is set to 1 millisecond in order 
to reuse the same socket to load multiple assets from the same host. When updating assets in the background, however, Nano Adblocker 
will wait 5 minutes between two requests to not slow down other connections, uBlock Origin will instead only wait for 2 minutes. 
